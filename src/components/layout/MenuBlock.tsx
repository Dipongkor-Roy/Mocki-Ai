...
const MenuBlock: React.FC<MenuBlockProps> = ({
  ...
  return (
    <>
        ...
        {menuItems.map((item, index) =>
          item.subMenu ? (
              ...
              <div className="absolute top-full left-0 w-full bg-white shadow-lg hidden group-hover:block rounded-lg z-50">
                <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-5 lg:gap-6 gap-1 lg:p-6 p-0 text-base font-medium text-gray-900">
                  ...
                  <div className="hidden lg:flex flex-col justify-between bg-blue-50 rounded-xl p-6">
                    ...
                    <a
                      href="blog-1.html"
                      className="text-blue-600 hover:translate-x-2 duration-300 transition font-medium inline-flex items-center gap-1 text-base"
                    >
                      See more content
            ...
            <li key={index}>
              <Link
                href={item.href || "#"}
                className="flex items-center gap-1 py-8"
              >
                {item.title}
              </Link>
            </li>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-white z-50 transform transition-transform duration-300 flex flex-col lg:hidden ${
          ...
          {menuItems.map((item, index) => (
            <li key={index}>
              {item.subMenu || item.megaMenu ? (
                ...
                <Link
                  href={item.href || "#"}
                  className="py-2 block text-gray-900 font-medium"
                >
                  {item.title}
                </Link>
              )}
